...
% \noindent
% {\em Dokaz:\/} Želimo pokazati, da sta množici $\Qe$ in $\left\{ \cos\theta + u\sin\theta; u \in \Ue, \theta \in \R \right\}$ enaki. To pokažemo tako, da pokažemo inkluzijo v obe smeri.
% Ker je desna inkluzija precej trivialna, naredimo to najprej:

% $\left\{ \cos\theta + u\sin\theta; u \in \Ue, \theta \in \R \right\} \subseteq \Qe$:

% \noindent
% Zadošča pogledati samo dolžino tako napisanega kvaterniona. Izračunamo torej
% \begin{center}
%    $ |\cos\theta + u\sin\theta|^2 = \cos^2\theta +|u|^2\sin^2\theta$. 
% \end{center}
% Ker je $u \in \Ue$, velja $|u| = 1$. V izračun vstavimo to enkost in dobimo
% \begin{center}
%    $ \cos^2\theta +1 \sin^2\theta = 1 $
% \end{center}

% \noindent
% Pogledamo še levo inkluzijo:

% $\Qe \subseteq \left\{ \cos\theta + u\sin\theta; u \in \Ue, \theta \in \R \right\}$:

% \noindent
% Naj bo $q \in \Qe$ poljuben. Želimo pokazati, da se $q$ da zapisati kot $\cos\theta + u\sin\theta$.
% Zapišemo $q = q_{0} + \vec{q}$. Spomnimo se, da sta $q_{0}$ in $\vec{q}$ pravokotna v $\H$ glede na definirani skalarni produkt.
% Če zožimo pogled na ravnino, ki jo razpenjata $q_{0}$ in $\vec{q}$, lahko vzamemo argument kvaterniona kot
% \begin{center}
%    $arg(q) = arctan\bigl(\dfrac{|\vec{q}|}{q_{0}}\bigr) := \theta$.
% \end{center}
% Prav tako iz te izbire in relacije $q_{0}^2 + |\vec{q}|^2 = 1$ sledi, da je $q_{0}^2 = \cos^2\theta$ in $|\vec{q}|^2 =\sin^2\theta$.
% Torej je $q_{0} = \cos\theta$, za $\vec{q}$ pa potrebujemo celoten vektorski del, denimo k, da $\vec{q} = k\sin\theta$. Ker je $\vec{q} \ in \H - \R$,
% sledi da je tudi $k \in \H - \R$. Pogledamo še dolžino:
% \begin{center}
%    $1 = |q|^2 = q_{0}^2 + |\vec{q}|^2 = \cos^2\theta + |k|^2\sin^2\theta$.
% \end{center}
% Iz te enakosti sledi, da je $|k| = 1$, in ker je $k$ čisti vektorski kvaternion, je torej čisti verson in
% $k \in \Ue$.
% \qed
